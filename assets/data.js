...
const H = {
  ...
  tickerName(code) {
    if (!code) return code;
    return window.TickerHelper?.getName(code) || code;
  },

  tickerSector(code) {
    if (!code) return code;
    return window.TickerHelper?.getSector(code) || "غير محدد";
  },
...
function normalizeTransactions(transactions) {
  return transactions.map(t => {
    ...
    // استخدام التحويل الموحد
    if (normalized.ticker) {
      normalized.ticker = window.TickerHelper?.normalizeCode(normalized.ticker) || normalized.ticker;
    }
    
    // لا نضيف name أبداً - سيتم جلبه من tickers.js عند العرض فقط
...
function normalizeDividends(dividends) {
  return dividends.map(d => {
    ...
    // لا نضيف stock أبداً - سيتم جلبه من tickers.js عند العرض فقط
